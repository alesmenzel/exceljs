...
class WorkbookWriter {
  ...
  _openStream(path) {
    // Just a pass through for archiver, we do not need to buffer anything here
    const stream = new PassThrough({highWaterMark: 1});

    console.log('Open stream %s', path);

    this.zip.append(stream, {name: path});
    stream.on('error', err => console.log('ERR', path, err));
  ...
  async _commitWorksheets() {
    ...
    return Promise.all(this._worksheets.map(commitWorksheet));
  }
